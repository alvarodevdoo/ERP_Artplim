...
export class CompanyService {
  ...
  async create(data: CreateCompanyDto): Promise<CompanyResponseDto> {
    try {
      ...
      if (!this.validateDocument(data.cnpj)) {
        throw new AppError('Documento inválido', 400);
      }

      // Verificar se documento já existe
      const documentExists = await this.companyRepository.documentExists(data.cnpj);
      if (documentExists) {
  ...
  async update(id: string, data: UpdateCompanyDto): Promise<CompanyResponseDto> {
    try {
      ...
      if (data.cnpj && !this.validateDocument(data.cnpj)) {
        throw new AppError('Documento inválido', 400);
      }

      // Verificar se documento já existe (excluindo a empresa atual)
      if (data.cnpj) {
        const documentExists = await this.companyRepository.documentExists(data.cnpj, id);
        if (documentExists) {
          throw new AppError('Documento já cadastrado', 409);
        }
      }
