...
export class AuthService {
  ...
  async login(data: LoginDto): Promise<LoginResponseDto> {
    ...
    try {
      ...
      logger.info(`Generated tokens for user ${user.email}: ${JSON.stringify(tokens)}`);
  ...
  private async generateTokens(userId: string, companyId: string): Promise<RefreshTokenResponseDto> {
    ...
    logger.info(`Server time (Date.now()): ${Date.now()}`);
    logger.info(`Payload before signing: ${JSON.stringify(payload)}`);

    const accessTokenOptions: SignOptions = {
      expiresIn: 4 * 60 * 60, // 4 hours in seconds
    };

    logger.info(`AccessTokenOptions: ${JSON.stringify(accessTokenOptions)}`);
