...
export async function productRoutes(fastify: FastifyInstance) {
  ...
  fastify.get<{
    ...
    async (request: FastifyRequest<{ Querystring: ProductFiltersDto }>, reply: FastifyReply) => {
      try {
        ...
        const query: ProductFiltersDto = {
          ...request.query,
          page: parseInt(String(request.query.page) || '1', 10),
          limit: parseInt(String(request.query.limit) || '10', 10),
        };

        const result = await productService.findMany(query, companyId, userId);
  ...
  fastify.patch<{
    Params: { id: string };
    Body: { resetStock?: boolean };
  }>(
    '/:id/reactivate',
    async (request, reply) => {
      try {
        const { companyId, userId } = request.user;
        const { resetStock = false } = request.body || {};
        const product = await productService.reactivate(
          request.params.id,
          companyId,
          userId,
          resetStock
  ...
  fastify.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      try {
        ...
        const { companyId, userId } = request.user;
        const stats = await productService.getOverallProductStats(companyId, userId);
