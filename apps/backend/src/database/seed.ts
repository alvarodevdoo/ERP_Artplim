...
async function main() {
  ...
  let officeFurnitureCategory = await prisma.productCategory.findFirst({
    where: {
  ...
  if (!officeFurnitureCategory) {
    officeFurnitureCategory = await prisma.productCategory.create({
      data: {
        name: 'Móveis de Escritório',
        description: 'Categorias para móveis de escritório',
        companyId: company.id,
        parentId: null,
      },
    });
  ...
  const mesa = await prisma.product.upsert({
    ...
    create: {
      ...
      sku: 'MESA-001',
      categoryId: officeFurnitureCategory.id,
      unit: 'UN',
      costPrice: 250.00,
      salePrice: 450.00,
      minStock: 5,
